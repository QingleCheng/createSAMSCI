...
Building::readBIM(const char *event, const char *bim)
{
  ...
  json_t *nameType = json_object_get(GI,"name");

  json_t *location = json_object_get(GI,"location");


  x = json_real_value(json_object_get(location,"latitude"));
  y = json_real_value(json_object_get(location,"longitude"));

  const char *type = json_string_value(sType);
  string s(type);
  strutype1=json_string_value(sType);
  name=json_string_value(nameType);
...
void
Building::writeSAMSCI(const char *path)
{
    ofstream outfile(path,ios::app);//ios::app表示在原文件末尾追加
//    outfile<<name<<"\t"<<name<<"\t"<<strutype1<<"\t"<<name<<"\t"<<year<<"\t"<<name<<"\t"<<nStory<<"\t"<<storyheight<<"\t"<<
//             area<<"\t"<<name<<"\t"<<name<<"\t"<<name<<"\n";
